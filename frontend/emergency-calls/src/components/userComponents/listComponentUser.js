...
function Row(props) {
  ...
  console.log("aga rows bunlar,", row.isRescuied);
  ...
  const fetchTeams = useCallback(async () => {
    try {
      ...
      const filteredTeams = teamsData.filter(
        (team) => !assignedTeams.includes(team.name) && !team.isOnTask
      );
  ...
  const handleConfirm = async () => {
    setConfirming(true);
    try {
      await handleEmergency(row.id, selectedTeam);
                ...
                <div>
                  ...
                  {selectedTab === 2 && (
                    <div className="mt-4">
                      ...
                      {isConfirmed || row.isRescuied != null ? (
